...
$(document).ready(function(){
    ...
    function listarProductos() {
        $.ajax({
            url: './backend/index.php?action=list',
            type: 'GET',
    ...
    $('#search').keyup(function() {
        if($('#search').val()) {
            ...
            $.ajax({
                url: './backend/index.php?action=search&search=' + $('#search').val(),
                data: {search},
                type: 'GET',
                success: function (response) {
                    if(!response.error) {
...
$('#name').on('keyup', function() {
    ...
    if (nombre.length > 0) {
        $.ajax({
            url: './backend/index.php?action=singlebyname',
            type: 'GET',
            data: { nombre: nombre ,
                //id: productId
            },
    ...
    $('#product-form').submit(e => {
        ...
        const url = edit === false ? './backend/index.php?action=add' : './backend/index.php?action=edit';
    ...
    $(document).on('click', '.product-delete', (e) => {
        if(confirm('¿Realmente deseas eliminar el producto?')) {
            ...
            $.post('./backend/index.php?action=delete', {id}, (response) => {
...
//editar producto
    $(document).on('click', '.product-item', (e) => {
        const element = $(this)[0].activeElement.parentElement.parentElement;
        const id = $(element).attr('productId');
        $.post('./backend/index.php?action=single', {id}, (response) => {
